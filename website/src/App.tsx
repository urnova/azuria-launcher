...
import { Download, Gamepad2, Zap, Shield, ChevronRight, Loader2 } from 'lucide-react'
    ...
    <div className="relative w-full min-h-screen overflow-hidden font-sans">
      ...
      <main className="relative z-10 flex flex-col items-center justify-center min-h-[80vh] px-6 text-center">
        ...
        <p className="text-lg md:text-xl text-gray-400 mb-10 max-w-2xl font-light">
          Découvrez notre nouveau launcher optimisé, avec mise à jour automatique des mods, support manette, et un gameplay en survie inédit.
        </p>

        <div className="flex flex-col sm:flex-row items-center gap-4 w-full justify-center max-w-md">
          {loading ? (
            <button disabled className="w-full sm:w-auto glass-panel px-8 py-4 rounded-2xl flex items-center justify-center gap-3 font-bold text-lg text-white opacity-80 cursor-wait">
              <Loader2 className="animate-spin w-6 h-6 text-[#4f8ef7]" />
              Recherche de version...
            </button>
          ) : downloadUrl ? (
            <a href={downloadUrl} className="w-full sm:w-auto px-8 py-4 rounded-2xl flex items-center justify-center gap-3 font-bold text-lg text-white hover:-translate-y-1 transition-all duration-300 relative group overflow-hidden" style={{ background: 'linear-gradient(135deg, #4f8ef7, #6ba3ff)', boxShadow: '0 10px 30px -10px rgba(79,142,247,0.6)' }}>
              ...
              <span className="relative z-10 flex flex-col items-start">
                ...
                <span className="text-[10px] font-normal opacity-80 uppercase tracking-widest">{version}</span>
              </span>
            </a>
          ) : (
            <button disabled className="w-full sm:w-auto glass-panel px-8 py-4 rounded-2xl flex items-center justify-center gap-3 font-bold text-lg text-white opacity-50 cursor-not-allowed">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          <div className="glass-panel p-8 rounded-3xl hover:-translate-y-2 transition-transform duration-300 group">
            <div className="w-14 h-14 rounded-2xl bg-[#4f8ef7]/10 flex items-center justify-center mb-6 border border-[#4f8ef7]/20 group-hover:scale-110 transition-transform">
              <Zap className="w-7 h-7 text-[#4f8ef7]" />
            </div>
            <h3 className="text-xl font-bold mb-3">Mises à jour fluides</h3>
            <p className="text-gray-400 text-sm leading-relaxed">Notre launcher télécharge et synchronise automatiquement les mods du serveur. Vous êtes toujours prêt à jouer.</p>
          </div>
          ...
          <div className="glass-panel p-8 rounded-3xl hover:-translate-y-2 transition-transform duration-300 group">
            <div className="w-14 h-14 rounded-2xl bg-[#aa44ff]/10 flex items-center justify-center mb-6 border border-[#aa44ff]/20 group-hover:scale-110 transition-transform">
              <Gamepad2 className="w-7 h-7 text-[#aa44ff]" />
            </div>
            <h3 className="text-xl font-bold mb-3">Support Manette Inclus</h3>
            <p className="text-gray-400 text-sm leading-relaxed">Jouez confortablement depuis votre canapé. Le mod Controllable est intégré et activable en un clic.</p>
          </div>
          ...
          <div className="glass-panel p-8 rounded-3xl hover:-translate-y-2 transition-transform duration-300 group">
            ...
            <h3 className="text-xl font-bold mb-3">Expérience Optimisée</h3>
            <p className="text-gray-400 text-sm leading-relaxed">Avec Embeddium et nos réglages sur-mesure, profitez d'une fluidité parfaite même sur des petites configurations.</p>
          </div>
        </div>
      </section>
          ...
          <div className="space-y-6 relative">
            ...
            <div className="flex gap-6 relative">
              ...
              <div>
                ...
                <p className="text-gray-400 text-sm">Cliquez sur le bouton de téléchargement plus haut pour récupérer l'installeur Windows (.exe).</p>
              </div>
            </div>
            ...
            <div className="flex gap-6 relative">
              ...
              <div>
                ...
                <p className="text-gray-400 text-sm">Ouvrez le Launcher Azuria, connectez-vous avec votre compte Premium ou Hors-ligne, et cliquez sur Jouer ! Les mods se téléchargeront tout seuls.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <footer className="relative z-10 border-t border-white/5 py-8 mt-12">
        <div className="container mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-4 text-xs text-gray-500 font-semibold">
          <div>© 2026 Azuria. Tous droits réservés.</div>
